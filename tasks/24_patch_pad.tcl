...
# Option --patch-pad: [3.xx/4.xx]  EMER_INIT: --> Patch emer_init.self to enable Usage of (wired only?) 3rd Party Controller in RECOVERY Menu
...
namespace eval ::24_patch_pad {
	...
	proc EMER_elf_Patch {elf} {
		...
		if {$::24_patch_pad::options(--patch-pad)} {
            log "Patching EMER_INIT (Reovery Menu) to enable (wired only?) 3rd-Party Controller"
			00
