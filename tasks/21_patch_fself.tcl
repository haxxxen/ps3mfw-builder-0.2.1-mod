...
namespace eval ::21_patch_fself {

    array set ::21_patch_fself::options {
		--version "REBUG D-REX 4.XX"
        --patch-fself true
    }
    ...
    proc main { } {	
		...
		if {$::21_patch_fself::options(--version) == ""} {
			return -code error "  YOU HAVE TO SELECT FIRMWARE BASE VERSION !!!"
		}

		if {${::NEWMFW_VER} >= "4.20"} {
			set vsh [file join dev_flash vsh module vsh.self]
				::modify_devflash_file $vsh ::21_patch_fself::Do_VSH_Patches

			if {($::21_patch_fself::options(--version) != "CEX 4.XX")} {
				set vshs {vsh/module/vsh.self.cexsp vsh/module/vsh.self.swp}
					::modify_devflash_files [file join dev_flash] $vshs ::21_patch_fself::Do_VSH_Patches
				if {($::21_patch_fself::options(--version) == "REBUG REX 4.XX")} {
					set lv2dex "lv2Dkernel.self"
						::modify_coreos_file $lv2dex ::21_patch_fself::Do_LV2_Patches
				} else {
					set lv2 "lv2Ckernel.self"
						::modify_coreos_file $lv2 ::21_patch_fself::Do_LV2_Patches
				}
			}
			...
			set file [file join $path $self]
				::modify_coreos_file $file ::21_patch_fself::Do_APPLDR_Patches
		} else {
			if {($::21_patch_fself::options(--version) == "3.41 / 3.55 CEX / REBUG")} {
				set self "appldr"
					::modify_coreos_file $self ::21_patch_fself::Do_APPLDR_Patches
			}
		}
    }
    ...
    proc Do_VSH_Patches {self} {
        log "Patching [file tail $self]"
			::modify_self_file $self ::21_patch_fself::VSH_elf_Patches
    }
    ...
    proc Do_LV2_Patches {self} {
        log "Patching [file tail $self]"
			::modify_sce_file $self ::21_patch_fself::LV2_elf_Patches
    }
    ...
    proc Do_APPLDR_Patches {self} {
        log "Patching [file tail $self]"
			::modify_iso_file $self ::21_patch_fself::APPLDR_elf_Patches
    }
