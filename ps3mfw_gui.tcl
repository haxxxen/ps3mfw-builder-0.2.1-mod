...
namespace eval ::gui {
    ...
    proc create_gui {arguments selected_tasks} {
        ...
        set input_button [::ttk::button .left.files.input.button -text "[::xml::GetData ${::xmllang} "Lang:Browse" 0]" -command [list ::gui::browse open "false" $input_entry [list { "PS3 Firmware Update" {*.* *.*} }] ".PUP" "Choose the Official Firmware Update"]]
    ...
    proc create_menu {} {
        ...
        $filemenu add command -label "[::xml::GetData ${::xmllang} "Lang:Select_Official_firmware" 0]" \
          -command [list ::gui::browse open "false" .left.files.input.entry [list { "PS3 Firmware Update" {*.* *.*} }] ".PUP" "Choose the Official Firmware Update"]
