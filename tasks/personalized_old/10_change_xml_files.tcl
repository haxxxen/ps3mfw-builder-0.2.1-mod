...
namespace eval 10_change_xml_files {

    array set ::10_change_xml_files::options {
    ...
    proc main {} {
        ...
        if {[file exists $options(--xml-game)] == 0 } {
            ...
            ::modify_devflash_file ${xml_game} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-game)
        }
		
		if {[file exists $options(--xml-friend)] == 0 } {
            log "Skipping xml_friend, $options(--xml-friend) does not exist"
        } else {
            ::modify_devflash_file ${xml_friend} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-friend)
        }
		
		if {[file exists $options(--xml-music)] == 0 } {
            log "Skipping xml_music, $options(--xml-music) does not exist"
        } else {
            ::modify_devflash_file ${xml_music} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-music)
        }
		
		if {[file exists $options(--xml-network)] == 0 } {
            log "Skipping xml_network, $options(--xml-network) does not exist"
        } else {
            ::modify_devflash_file ${xml_network} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-network)
        }
		
		if {[file exists $options(--xml-photo)] == 0 } {
            log "Skipping xml_photo, $options(--xml-photo) does not exist"
        } else {
            ::modify_devflash_file ${xml_photo} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-photo)
        }
		
		if {[file exists $options(--xml-psn)] == 0 } {
            log "Skipping xml_psn, $options(--xml-psn) does not exist"
        } else {
            ::modify_devflash_file ${xml_psn} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-psn)
        }

        if {[file exists $options(--xml-sysconf)] == 0 } {
            log "Skipping xml_sysconf, $options(--xml-sysconf) does not exist"
        } else {
            ::modify_devflash_file ${xml_sysconf} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-sysconf)
        }
		
		if {[file exists $options(--xml-tv)] == 0 } {
            log "Skipping xml_tv, $options(--xml-tv) does not exist"
        } else {
            ::modify_devflash_file ${xml_tv} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-tv)
        }
		
		if {[file exists $options(--xml-user)] == 0 } {
            log "Skipping xml_user, $options(--xml-user) does not exist"
        } else {
            ::modify_devflash_file ${xml_user} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-user)
        }
		
		if {[file exists $options(--xml-video)] == 0 } {
            log "Skipping xml_video, $options(--xml-video) does not exist"
        } else {
            ::modify_devflash_file ${xml_video} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-video)
        }
		
		if {[file exists $options(--xml-ps2)] == 0 } {
            log "Skipping xml_ps2, $options(--xml-ps2) does not exist"
        } else {
            ::modify_devflash_file ${xml_ps2} ::10_change_xml_files::copy_xml_file $::10_change_xml_files::options(--xml-ps2)
        }
    }
