...
namespace eval 06_change_devflash_files {

    array set ::06_change_devflash_files::options {
        --change-filenames "dev_flash/path/to/file"
    }
        ...
        foreach file [split $options(--change-filenames) "\n"] {
            if {[string equal -length 14 "dev_flash/path" ${file}] != 1} {
                if {[string equal -length 10 "dev_flash/" ${file}] == 1} {
                    ::modify_devflash_file ${file} ::06_change_devflash_files::change_file
                }
            }
        }
    }
