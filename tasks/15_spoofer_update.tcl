...
# Type --version: combobox { {CEX} {DEX} {REBUG} {CODREX} }
...
namespace eval ::15_spoofer_update {

    array set ::15_spoofer_update::options {
      --version ""
      --spoof ""
      --psn ""
      --screen false
    ...
    proc main {} {
		variable options
		if {$::15_spoofer_update::options(--spoof) == ""} {
			return -code error "  YOU HAVE NOT SELECTED SPOOFER VERSION !!!"
		} else {
			if {$::15_spoofer_update::options(--version) != ""} {
				if {$::15_spoofer_update::options(--version) == "CEX"} {
					log "Patching CEX vsh.self with new Build and Version Number"
						::modify_devflash_file [file join dev_flash vsh module vsh.self] ::15_spoofer_update::patch_vsh_cex
					log "Patching version.txt & index.dat files to spoof all entries"
						::modify_devflash_file [file join dev_flash vsh etc version.txt] ::15_spoofer_update::version_txt
				} elseif {$::15_spoofer_update::options(--version) == "DEX"} {
					log "Patching DEX vsh.self with new Build and Version Number"
						::modify_devflash_file [file join dev_flash vsh module vsh.self] ::15_spoofer_update::patch_vsh_dex
					log "Patching version.txt & index.dat files to spoof all entries"
						::modify_devflash_file [file join dev_flash vsh etc version.txt] ::15_spoofer_update::version_txt
				} elseif {$::15_spoofer_update::options(--version) == "REBUG"} {
					if {${::NEWMFW_VER} > "4.46"} {
						log "Patching REBUG's vsh.self files with new Build and Version Number"
							::modify_devflash_file [file join dev_flash vsh module vsh.self.cexsp] ::15_spoofer_update::patch_vsh_cex
							set selfs {vsh.self vsh.self.swp}
							::modify_devflash_files [file join dev_flash vsh module] $selfs ::15_spoofer_update::patch_vsh_dex
						log "Patching version.txt & index.dat files to spoof all entries"
							::modify_devflash_file [file join dev_flash vsh etc version.txt] ::15_spoofer_update::version_txt
							::modify_devflash_file [file join dev_flash vsh etc version.txt.swp] ::15_spoofer_update::version_txt_swp
					} else {
						log "Patching REBUG's vsh.self.cexsp & vsh.self.swp files with new Build and Version Number"
							::modify_devflash_file [file join dev_flash vsh module vsh.self.cexsp] ::15_spoofer_update::patch_vsh_cex
							::modify_devflash_file [file join dev_flash vsh module vsh.self.swp] ::15_spoofer_update::patch_vsh_dex
						log "Patching version.txt.swp & index.dat.swp files to spoof all entries"
							::modify_devflash_file [file join dev_flash vsh etc version.txt.swp] ::15_spoofer_update::version_txt_swp
					}
				} elseif {$::15_spoofer_update::options(--version) == "CODREX"} {
						log "Patching REBUG's vsh.self.cexsp & vsh.self.swp files with new Build and Version Number"
							::modify_devflash_file [file join dev_flash vsh module vsh.self.cexsp] ::15_spoofer_update::patch_vsh_cex
							::modify_devflash_file [file join dev_flash vsh module vsh.self] ::15_spoofer_update::patch_vsh_dex
						log "Patching version.txt.swp & index.dat.swp files to spoof all entries"
		...
		if {$::15_spoofer_update::options(--psn) != ""} {
			if {${::NEWMFW_VER} < "4.66"} {
				if {$::15_spoofer_update::options(--version) != ""} {
					if {$::15_spoofer_update::options(--version) == "REBUG"} {
						log "Patching REBUG's vsh.self.cexsp & vsh.self.swp to enable SEN on lower MFW/CFW again."
							...
							::modify_devflash_files [file join dev_flash vsh module] $selfs ::15_spoofer_update::patch_psn
					}
					if {$::15_spoofer_update::options(--version) == "CODREX"} {
						log "Patching REBUG's vsh.self.cexsp & vsh.self.swp to enable SEN on lower MFW/CFW again."
							set selfs {vsh.self.cexsp vsh.self}
							::modify_devflash_files [file join dev_flash vsh module] $selfs ::15_spoofer_update::patch_psn
					} else {
						log "Patching vsh.self to enable SEN on lower MFW/CFW again."
							::modify_devflash_file [file join dev_flash vsh module vsh.self] ::15_spoofer_update::patch_psn
					}
				}
			}
		}
		if {$::15_spoofer_update::options(--screen)} {
			...
			if {$::15_spoofer_update::options(--version) != ""} {
				if {$::15_spoofer_update::options(--version) == "REBUG"} {
					log "Patching explore_category_photo.xml and REBUG's vsh.self file(s) to enable screenshot feature"
						...
						::modify_devflash_file $xml ::15_spoofer_update::callback_patch
						::modify_devflash_files [file join dev_flash vsh module] $selfs ::15_spoofer_update::patch_screen
				} else {
					log "Patching explore_category_photo.xml and vsh.self file to enable screenshot feature"
						::modify_devflash_file $xml ::15_spoofer_update::callback_patch
						::modify_devflash_file [file join dev_flash vsh module vsh.self] ::15_spoofer_update::patch_screen
				}
			}
		}
    }
    ...
    proc patch_vsh_cex {self} {
		::modify_self_file $self ::15_spoofer_update::patch_cex_elf
	}
    proc patch_vsh_dex {self} {
		::modify_self_file $self ::15_spoofer_update::patch_dex_elf
	}
	...
	proc callback_patch {path args} {		
		...
		if {$::15_spoofer_update::options(--patch-screen)} {
    ...
    proc patch_screen {self} {
		::modify_self_file $self ::15_spoofer_update::screen_elf
	}
    proc screen_elf {elf} {
		log "Patch 1" 1
    ...
    proc patch_psn {self} {
		::modify_self_file $self ::15_spoofer_update::psn_elf
	}
    proc psn_elf {elf} {
		if {$::15_spoofer_update::options(--psn) == "Patch 1"} {
			log "SEN Enabler Patch 1 (credits to ps3ita team)"
				set search  "\x2F\x80\x00\x02\x40\x9E\x00\x3C\x48\x00\x00\x10"
				set replace "\x2F\x80\x00\x02\x60\x00\x00\x00\x60\x00\x00\x00"
				set offset 0
				set mask 0			
				# PATCH THE ELF BINARY
					catch_die {::patch_elf $elf $search $offset $replace $mask} "Unable to patch self [file tail $elf]"
		...
		if {$::15_spoofer_update::options(--psn) == "Patch 2"} {
			log "SEN Enabler Patch 2 (credits to Evilnat)"
				set search  "\x80\x1E\x00\x40\x2F\x80\x00\x00"
				set replace "\x38\x00\x00\x00\x2F\x80\x00\x00"
				set offset 0
		...
		if {$::15_spoofer_update::options(--psn) == "Patch 3"} {
