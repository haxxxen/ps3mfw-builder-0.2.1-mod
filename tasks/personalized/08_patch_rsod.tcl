...
namespace eval ::08_patch_rsod {
    ...
    array set ::08_patch_rsod::options {
        --patch-rsod-version ""
    }
    proc main {} {
		if {$::08_patch_rsod::options(--patch-rsod-version) != ""} {
			set self [file join dev_flash vsh module basic_plugins.sprx]
			::modify_devflash_file $self ::08_patch_rsod::patch_self
		}
    }

    proc patch_self {self} {
        log "Patching [file tail $self]"
		::modify_self_file $self ::08_patch_rsod::patch_elf
    }
	
    proc patch_elf {elf} {
        if {$::08_patch_rsod::options(--patch-rsod-version) == "3.15"} {
			debug "Patching [file tail $elf] to disable RSOD Screen on ${::08_patch_rsod::options(--patch-rsod-version)}"
        ...
        if {$::08_patch_rsod::options(--patch-rsod-version) == "3.15 DEX"} {
			debug "Patching [file tail $elf] to disable RSOD Screen on ${::08_patch_rsod::options(--patch-rsod-version)}"
        ...
        if {$::08_patch_rsod::options(--patch-rsod-version) == "3.40-3.55"} {
			debug "Patching [file tail $elf] to disable RSOD Screen on ${::08_patch_rsod::options(--patch-rsod-version)}"
        ...
        if {$::08_patch_rsod::options(--patch-rsod-version) == "4.xx"} {
			debug "Patching [file tail $elf] to disable RSOD Screen on ${::08_patch_rsod::options(--patch-rsod-version)}"
