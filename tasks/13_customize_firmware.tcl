...
namespace eval 13_customize_firmware {

    array set ::13_customize_firmware::options {
    ...
    proc main {} {
	    ...
	    ::13_customize_firmware::set_var_cf
        
        if {[file exists $::13_customize_firmware::options(--customize-system-sound-rco)] == 0 } { 
            log "Skipping xmb sounds, $::13_customize_firmware::options(--customize-system-sound-rco) does not exist"
        } else {
            ::modify_devflash_file $xmbSoundRCO ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-system-sound-rco)
        }

        if {[file exists $::13_customize_firmware::options(--customize-coldboot-raf)] == 0 } {
            log "Skipping coldboot.raf, $::13_customize_firmware::options(--customize-coldboot-raf) does not exist"
        } else {
            ::modify_devflash_file $coldboot_raf ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-coldboot-raf)
        }

        if {[file exists $::13_customize_firmware::options(--customize-coldboot-stereo)] == 0 } {
            log "Skipping coldboot_stereo, $::13_customize_firmware::options(--customize-coldboot-stereo) does not exist"
        } else {
            ::modify_devflash_file $coldboot_stereo ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-coldboot-stereo)
        }

        if {[file exists $::13_customize_firmware::options(--customize-coldboot-multi)] == 0 } {
            log "Skipping coldboot_multi, $::13_customize_firmware::options(--customize-coldboot-multi) does not exist"
        } else {
            ::modify_devflash_file $coldboot_multi ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-coldboot-multi)
        }
        
        if {[file exists $::13_customize_firmware::options(--customize--health-screen)] == 0 } {
            log "Skipping health_screen, $::13_customize_firmware::options(--customize--health-screen) does not exist"
        } else {
            ::modify_rco_file $health_screen ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize--health-screen)
        }
		
		if {[file exists $::13_customize_firmware::options(--customize-firmware-xmb-wave)] == 0 } {
            log "Skipping xmb_wave, $::13_customize_firmware::options(--customize-firmware-xmb-wave) does not exist"
        } else {
            ::modify_devflash_file $xmb_wave ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-firmware-xmb-wave)
        }
        
        if {[file exists $::13_customize_firmware::options(--customize-game-boot-rco)] == 0 } {
            log "Skipping Game Boot logos, $::13_customize_firmware::options(--customize-game-boot-rco) does not exist"
        } else {
            ::modify_devflash_file $gameBootRCO ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-game-boot-rco)
        }
        
        if {[file exists $::13_customize_firmware::options(--customize-theme-src)] == 0} {
            log "Skipping theme, $::13_customize_firmware::options(--customize-theme-src) does not exist"
        } else {
            ::modify_devflash_file $theme ::13_customize_firmware::copy_customized_file $::13_customize_firmware::options(--customize-theme-src)
        }
        
        if {$::13_customize_firmware::options(--customize-fw-version) != ""} {
            log "Patching fw version in XMB settings"
            ::modify_rco_file $fw_version ::13_customize_firmware::callback_fwversion
        }
        ...
        if {[file exists $lp] != 0} {
            ...
            extract_tar $::13_customize_firmware::options(--language-pack) $langpackDir
	...
	proc set_var_cf { } {
	    variable options
        set arg $::13_customize_firmware::options(--customize-embedded-app)
        set lp $::13_customize_firmware::options(--language-pack)
    ...
    proc callback_fwversion { path args } {
        ...
        sed_in_place [file join $path English.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching German.xml into [file tail $path]"
        sed_in_place [file join $path German.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Korean.xml into [file tail $path]"
        sed_in_place [file join $path Korean.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Russian.xml into [file tail $path]"
        sed_in_place [file join $path Russian.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Swedish.xml into [file tail $path]"
        sed_in_place [file join $path Swedish.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Spanish.xml into [file tail $path]"
        sed_in_place [file join $path Spanish.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Portugese.xml into [file tail $path]"
        sed_in_place [file join $path Portugese.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Norwegian.xml into [file tail $path]"
        sed_in_place [file join $path Norwegian.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Japanese.xml into [file tail $path]"
        sed_in_place [file join $path Japanese.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Italian.xml into [file tail $path]"
        sed_in_place [file join $path Italian.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching French.xml into [file tail $path]"
        sed_in_place [file join $path French.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Finnish.xml into [file tail $path]"
        sed_in_place [file join $path Finnish.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Dutch.xml into [file tail $path]"
        sed_in_place [file join $path Dutch.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching Danish.xml into [file tail $path]"
        sed_in_place [file join $path Danish.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching ChineseTrad.xml into [file tail $path]"
        sed_in_place [file join $path ChineseTrad.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
        
        log "Patching ChineseSimpl.xml into [file tail $path]"
        sed_in_place [file join $path ChineseSimpl.xml] %1.1 %1.1$::13_customize_firmware::options(--customize-fw-version)
    ...
    proc callback_rco {path src mode name} {
        variable options
        if {$mode == "0" } {
            set dst [file join $path $::13_customize_firmware::options(--language-replace).xml]
		}
