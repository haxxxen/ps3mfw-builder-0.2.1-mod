...
namespace eval ::14_patch_vsh {

    array set ::14_patch_vsh::options {
    ...
    proc main { } {
        variable options
        set src $::14_patch_vsh::options(--customize-fw-ssl-cer)
        set dst $::14_patch_vsh::options(--customize-fw-change-cer)
        set path [file join dev_flash data cert]
				
		
     
		# if "retail/debug pkg" options, then patch "nas_plugin.sprx"
        if {$::14_patch_vsh::options(--allow-pseudoretail-pkg) || $::14_patch_vsh::options(--allow-debug-pkg) || $::14_patch_vsh::options(--allow-retail-pkg-dex)} {
            set self [file join dev_flash vsh module nas_plugin.sprx]
            ::modify_devflash_file $self ::14_patch_vsh::patch_self
        }
        # if "unsigned/psn" patches enabled, patch "vsh.self"
        if {$::14_patch_vsh::options(--allow-unsigned-app) || $::14_patch_vsh::options(--patch-vsh-react-psn-v2-4x) || $::14_patch_vsh::options(--patch-rogero-vsh-patches) ||
			$::14_patch_vsh::options(--patch-vsh-no-delete-actdat)} {
            set self [file join dev_flash vsh module vsh.self]
            ::modify_devflash_file $self ::14_patch_vsh::patch_self
        }		
		
        # if "--customize-fw-ssl-cer" is defined, patch it
        if {$::14_patch_vsh::options(--customize-fw-ssl-cer) != ""} {
            if {[file exists $src] == 0 } {
                ...
                ::modify_devflash_files $path $dst ::14_patch_vsh::copy_customized_file $src
    ...
    proc patch_self {self} { 		
		::modify_self_file $self ::14_patch_vsh::patch_elf		
    }
    ...
    proc patch_elf {elf} {
		...
		if { [string first "nas_plugin.sprx" $elf 0] != -1 } {		
			...
			if {$::14_patch_vsh::options(--allow-pseudoretail-pkg) } {
			...
			if {$::14_patch_vsh::options(--allow-retail-pkg-dex) } {
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #	
				#
				# verified OFW ver. 3.55 - 4.55+
			...
			if {$::14_patch_vsh::options(--allow-debug-pkg) } {
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #						
				#
				# verified OFW ver. 3.55 - 4.55+
		...
		if { [string first "vsh.self" $elf 0] != -1 } {
			...
			if {$::14_patch_vsh::options(--patch-rogero-vsh-patches)} {	
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #			
				# (patch seems fine, NO mask req'd)
				#
			...
			if {$::14_patch_vsh::options(--allow-unsigned-app)} {
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #			
				# (patch seems fine, NO mask req'd)
				#
				# verified OFW ver. 3.55 - 4.55+
			...
			if {$::14_patch_vsh::options(--patch-vsh-react-psn-v2-4x)} {
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #							
				#
				# verified OFW ver. 3.55 - 4.55+
			...
			if {$::14_patch_vsh::options(--patch-vsh-no-delete-actdat)} {
				# <><> --- OPTIMIZED FOR 'PATCHTOOL' --- <><> #							
				#
				# verified OFW ver. 3.55 - 4.55+
