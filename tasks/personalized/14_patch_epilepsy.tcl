...
namespace eval ::14_patch_epilepsy {
    ...
    proc main {} {
		if {$::14_patch_epilepsy::options(--patch-disable-epilepsy-message) != ""} {
			if {$::14_patch_epilepsy::options(--patch-disable-epilepsy-message) == "CEX"} {
				::modify_devflash_file [file join dev_flash vsh module vsh.self] ::14_patch_epilepsy::patch_cex_self
			} elseif {$::14_patch_epilepsy::options(--patch-disable-epilepsy-message) == "DEX"} {
				::modify_devflash_file [file join dev_flash vsh module vsh.self] ::14_patch_epilepsy::patch_self
			} elseif {$::14_patch_epilepsy::options(--patch-disable-epilepsy-message) == "REBUG"} {
    ...
    proc patch_cex_elf { elf } {
		if {${::NEWMFW_VER} >= "4.21"} {
			...
			set replace "\x38\x00\x00\x01\x3C\x60\x00\x00"
			set offset 52
			set mask 0			
				# PATCH THE ELF BINARY
					catch_die {::patch_elf $elf $search $offset $replace $mask} "Unable to patch self [file tail $elf]"
