...
namespace eval 08_change_devflash_files {

    array set ::08_change_devflash_files::options {
...
dev_flash/vsh/resource/autodownload_plugin.rco
        ...
        foreach file [split $options(--change-filenames) "\n"] {
            if {[string equal -length 14 "dev_flash/path" ${file}] != 1} {
                if {[string equal -length 10 "dev_flash/" ${file}] == 1} {
                    ::modify_devflash_file ${file} ::08_change_devflash_files::change_file
                }
            }
        }
    }
